...
pub struct OrderedBlock {
    ...
    pub randomness: U256,
    /// GRETH-042: Indices of validators who failed to propose in the gap since the last block.
...
struct PipeExecService<Storage: GravityStorage> {
    ...
    ordered_block_rx: Receiver<ReceivedBlock>,
...
struct Core<Storage: GravityStorage> {
    ...
    /// GRETH-036: Bounded channel for backpressure
    execution_result_tx: Sender<ExecutionResult>,
    ...
    /// GRETH-036: Bounded channel for backpressure
    discard_txs_tx: Sender<Vec<TxHash>>,
...
impl<Storage: GravityStorage> Core<Storage> {
    /// GRETH-032: Extract precompile state changes (e.g. native mints) into accumulated state.
    /// This must be called before any epoch-change early return to prevent token loss.
    fn merge_precompile_state(
        state_for_precompile: &Arc<parking_lot::Mutex<ParallelState>>,
        accumulated_state_changes: &mut revm::state::EvmState,
    ) {
    ...
    async fn process(&self, block: ReceivedBlock) {
        ...
        if epoch > block_epoch {
            ...
            // GRETH-057: Use SeqCst to ensure epoch and execute_height are consistent
    ...
    async fn verify_executed_block_hash(&self, execution_result: ExecutionResult) -> Option<()> {
        ...
        // GRETH-036: Bounded channel — use .send().await for backpressure
        self.execution_result_tx.send(execution_result).await.ok()?;
        let block_hash = self.verified_block_hash_rx.wait(block_id).await?;
        // GRETH-052+053: Structured error logging and stricter verification
        match block_hash {
            Some(verified_hash) => {
    ...
    fn execute_system_transactions(
        ...
        // GRETH-042: Pass failed proposer indices from consensus layer
        ...
        // GRETH-033: Flush precompile state into EVM state after metadata txn
        // so subsequent system txns see any minted balances.
        {
            ...
            if !precompile_changes.is_empty() {
                evm.db_mut().commit(precompile_changes);
            }
        }
        ...
        if let Some((new_epoch, validators)) = metadata_txn_result.emit_new_epoch() {
            ...
            // GRETH-032: Merge any precompile state (e.g., native mints) before returning
            // to prevent token loss during epoch transitions.
            {
                let mut precompile_changes = revm::state::EvmState::default();
                Self::merge_precompile_state(&state_for_precompile_ref, &mut precompile_changes);
        ...
        for (index, extra_data) in sorted_extra_data.iter().enumerate() {
            ...
            // GRETH-033: Flush precompile state into EVM state after each validator txn
            // so subsequent txns see any minted balances.
            {
                let mut precompile_changes = revm::state::EvmState::default();
                Self::merge_precompile_state(&state_for_precompile_ref, &mut precompile_changes);
                if !precompile_changes.is_empty() {
            ...
            // GRETH-051: Deep-merge storage slots instead of overwriting entire account
        ...
        // Extract changes from precompile state and merge into accumulated_state_changes
        {
            ...
            // GRETH-050: Convert BundleState to EvmState, preserving original_value
            for (address, account) in precompile_bundle.state {
                if let Some(info) = account.info {
                    use revm::state::{Account, AccountStatus, EvmStorageSlot};
                    accumulated_state_changes.insert(
...
pub struct PipeExecLayerApi<Storage, EthApi> {
    /// GRETH-036: Bounded channel for backpressure
    ordered_block_tx: Sender<ReceivedBlock>,
    execution_result_rx: Mutex<Receiver<ExecutionResult>>,
...
impl<Storage: GravityStorage, EthApi> PipeExecLayerApi<Storage, EthApi> {
    ...
    pub fn push_ordered_block(&self, block: OrderedBlock) -> Option<()> {
        // GRETH-036: Bounded channel — use try_send for sync context
        match self.ordered_block_tx.try_send(ReceivedBlock::OrderedBlock(block)) {
            Ok(()) => Some(()),
            Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                warn!(target: "PipeExecLayerApi", "ordered block channel full — applying backpressure");
                None
            }
            Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
                error!(target: "PipeExecLayerApi", "ordered block channel closed");
                None
            }
        }
    }
    ...
    pub fn push_history_block(&self, block: RecoveredBlock<Block>) -> Option<()> {
        // GRETH-036: Bounded channel — use try_send for sync context
        self.ordered_block_tx.try_send(ReceivedBlock::HistoryBlock(block.into())).ok()
    }
    ...
    /// Returns `None` if the channel has been closed or persistence times out.
    pub async fn wait_for_block_persistence(&self, block_number: u64) -> Option<()> {
        use std::time::Duration;
        use tokio::time::timeout;

        // GRETH-054: Timeout to prevent indefinite blocking if persistence hangs
...
where
    ...
    EthApi::NetworkTypes: RpcTypes<TransactionRequest = TransactionRequest>,
{
    // GRETH-036: Use bounded channels to prevent unbounded memory growth from backpressure
    ...
    let (discard_txs_tx, discard_txs_rx) = tokio::sync::mpsc::channel(DISCARD_TXS_CHANNEL_CAP);
    ...
    // GRETH-037: Use typed OnceLock instead of Box<dyn Any>
    let _ = PIPE_EXEC_LAYER_EVENT_BUS.set(PipeExecLayerEventBus {
