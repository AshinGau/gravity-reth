        ...
        + StageCheckpointWriter,
{
    ...
    fn execute(&mut self, provider: &Provider, input: ExecInput) -> Result<ExecOutput, StageError> {
        ...
        let (trie_root, entities_checkpoint) = if range.is_empty() {
            ...
            // to avoid loading the entire state into memory, 128 kv entries for a block
            let chunk_entries_threshold = incremental_threshold as usize * 128;  
            ...
            for account_entry in account_cursor.walk(None)? {
                ...
                if let Some((found_key, entry)) = storage_cursor.seek(hashed_address)? {
                    if found_key == hashed_address {
                        if !entry.value.is_zero() {
                            storage.storage.insert(entry.key, entry.value);
                            chunk_num_entries += 1;
                        }
                        while let Some((found_key, entry)) = storage_cursor.next()? {
                            if found_key != hashed_address {
                                break;
                            }
                            if !entry.value.is_zero() {
