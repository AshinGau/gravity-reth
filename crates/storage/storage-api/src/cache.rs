...
use reth_trie::{nested_trie::Node, Nibbles, TrieInputV2};
use revm::{
...
pub struct PersistBlockCacheInner {
    ...
    deamon_handle: Mutex<Option<JoinHandle<()>>>,
}
...
impl PersistBlockCache {
    pub fn new() -> Self {
        ...
        inner.deamon_handle.lock().unwrap().replace(handle);
    ...
    pub fn write_trie_updates(&self, input: &TrieInputV2, block_number: u64) {
        let TrieInputV2 {
